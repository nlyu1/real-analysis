...
Given a measurable space $(X, \mca S)$. A function 
$X\to \R^n$ is $\mca S$-measurable if 
$f^{-1}(B) \in \mca S$ 
...
:::{.example}
If $\mca S=\{\emptyset, X\}$, then the only $\mca S$-measurable 
functions from $X\to \R^n$ are constant functions. On the 
other extreme, if $\mca S=2^X$, then every function is measurable. 
For a nontrivial example, if $\mca S=\{\emptyset, (-\infty, 0), [0, \infty), \R\}$ 
then $f:\R\to \R$ is measurable iff it is 
constant on $(-\infty, 0), [0, \infty)$. 
:::

:::{.definition name="characteristic function"}
Given $E\subset X$, the characteristic function $\chi_E:X\to \R$ is 
\[ 
    \chi_E(x) = \begin{cases}
        1 & x\in E \\ 
        0 & x\notin E
...
Given $(X, \mca S), E\subset X$, then $\chi_E$ is $\mca S$-measurable 
iff $E\in \mca S$. 
:::
_Proof:_ 
$\chi_E^{-1}(B) = \{E, X-E, X, \emptyset\}$ 
...
\] 
given a measurable space $(X, \mca S)$ and function $f:X\to \R^n$, 
\[ 
    f^{-1}(A) \in \mca S, \quad \forall A\in\mca A \implies 
    f \quad \mca S\text{-measurable}. 
\] 
:::
_Proof:_ Given $\mca A, f$, consider the subsets 
whose preimage are measurable 
\[  
    \mca T = \{B\subset \R^n| f^{-1}(B)\in \mca S\} \supset \mca T
\] 
...
$\emptyset = f^{-1}(\emptyset) = \emptyset \in \mca S$. 
For complement, given $B\in \mca T: f^{-1}(\R^n-B) = X-f^{-1}(B) \in \mca S$; 
...
Every continuous function $f:X\to \R^n$ defined on 
a Borel subset $X\subset \R^m$ is a Borel-measurable function. 
:::
_Proof:_ Consider the preimage of $x>a$ ($x, a\in \R^n$ and inequality 
holds elementwise). For any such $a$, consider 
\[ 
    f^{-1}(x>a) 
    = \left[\bigcup_{x\in f^{-1}(x>a)} (x-\delta_x, x+\delta_x) \right]\cap X
\] 
...
is an open set, hence its intersection with the Borel set $X$ is Borel. 

:::{.proposition name="composition preserves measurability"}
Given measurable $f:(X, \mca S)\to \R^n$ and $g:\R^n\to \R^m$ 
measurable, then $g\circ f$ is $\mca S$-measurable. 
:::
_Proof:_ Evident from the definition of measurability. 

$f:(X\subset \R)\to \R$ is increasing if $x < y\implies f(x)\leq f(y)$; 
it is monotonic if $f$ or $-f$ is increasing. 

:::{.proposition}
if $f:X\to \R$ is monotonic and $X$ is Borel, then $f$ is measurable. 
:::
...
Suppose $\{f_j\}\to f:(X, \mca S)\to \R^n$ point-wise each $f_j$ 
is $\mca S$-measurable, then $f$ is $\mca S$-measurable. 
:::
_Proof:_ Unrolling the definition of the limit: for any 
$x\in X, a\in \R^n$, $f(x)>a$ iff 
\begin{align}
...
Given measurable $\{f_k: (X, \mca S)\to \R\}$, the following functions 
are also measurable: 
\[ 
...
Given a measurable space $(X, \mca S)$, a 
measure on $(X, \mca S)$ is a function 
$\mu:\mca S\to [0, \infty]$ with 
\[ 
    \mu \left(\bigsqcup_{k=1}^\infty E_k\right) 
    = \sum_{k=1}^\infty \mu(E_k), \quad \mu(\emptyset) = 0 
\] 
for every disjoint sequence $\{E_j\} \in \mca S$. 
A measure space is an ordered triple $(X, \mca S, \mu)$. 
:::

:::{.proposition name="elementary properties of measures"}
Given a measure space $(X, \mca S, \mu)$ and $D\subset E\in \mca S$: 
...
Under a measure space $(X, \mca S, \mu)$ with a nested 
...
:::{.definition name="σ-finiteness"}
A measure space $(X, \Sigma, \mu)$ is $\sigma$-finite 
if there are countably many sets $A_1, \cdots$ such that 
\[ 
    \mu(A_{\forall j}) < \infty, \quad X = \bigcup_j A_j
\] 
...
Given a real function $f:X\to \R$, there exists a sequence 
